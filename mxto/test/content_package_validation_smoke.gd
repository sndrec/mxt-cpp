...
func _initialize() -> void:
	...
	var snapshot: MxtContentLoadResult = snapshot_catalog.snapshot_draft_package(vehicle_root, snapshot_library)
	_expect(snapshot.is_valid(), "vehicle package should become a registered test-drive snapshot: %s" % [snapshot.errors])
	if snapshot.is_valid():
		var snapshot_record := snapshot.record
		...
		_expect(snapshot_record != null and snapshot_record.root_path == snapshot.package_path, "test-drive record should point at the immutable snapshot")
		var repeated_snapshot: MxtContentLoadResult = snapshot_catalog.snapshot_draft_package(vehicle_root, snapshot_library)
		_expect(repeated_snapshot.is_valid() and repeated_snapshot.reused_existing, "same-session test-drive snapshot should reuse its validated catalog record")
	...
	if bool(import_result.get("valid", false)):
		...
		var scan_result: MxtContentLoadResult = catalog.scan_local_library(library_root)
		_expect(scan_result.is_valid(), "content-addressed local library should scan without diagnostics")
		...
		var workshop_add: MxtContentLoadResult = catalog.add_workshop_package(import_result["package_path"], 123456)
		_expect(workshop_add.is_valid(), "validated package should register as a Workshop record")
