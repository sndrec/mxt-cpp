class_name TranslateGizmo extends Node3D
...
var gizmo_material := preload("res://asset/mat/translate_gizmo_material.tres")
	...
	if !is_moving:
		...
		if Input.is_action_just_pressed("LeftMouse"):
			...
			if mouse_cast.is_colliding() and mousecast_wants_this_gizmo():
				...
				var offset_pos := cam.global_position - global_position
		...
		if intersect_pos is Vector3 and origin_point is Vector3:
			...
			if Input.is_action_pressed("Ctrl"):
				if Input.is_action_pressed("Shift"):
					...
					desire_point = original_transform.origin + snapped((intersect_pos - origin_point).project(use_vector), Vector3(12.5, 12.5, 12.5))
			global_position = desire_point
			target_node.global_position = global_position
