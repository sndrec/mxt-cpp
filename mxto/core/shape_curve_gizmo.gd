...
func _entry_lane_tx(entry_index : int) -> float:
	if curve_entries.size() <= 1:
		return 0.0
	return lerpf(-0.72, 0.72, float(entry_index) / float(curve_entries.size() - 1))

func _collect_curve_entries() -> void:
	curve_entries.clear()
	if !is_instance_valid(target_path) or !target_path.road_shape:
		return
	var shape := target_path.road_shape
	if shape is RoadShapeCylinderOpen or shape is RoadShapePipeOpen or shape is RoadShapeRoundedSquareOpen:
		curve_entries.append({
			"name": "openness",
			"curve": shape.openness,
			"min": 0.0,
		})
	if shape is RoadShapeRoundedSquare:
		curve_entries.append({
			"name": "width",
			"curve": shape.width,
			"min": 0.05,
	...
	if shape is RoadShapeRoundedSquareOpen:
		curve_entries.append({
			"name": "open_rotation",
			"curve": shape.open_rotation,
			"min": -INF,
		})
...
func _surface_normal(tx : float, ty : float) -> Vector3:
	...
	if samples.size() < 3:
		return Vector3.UP
	var tangent_x : Vector3 = (samples[1] - samples[0]) if clamped_tx < 0.0 else (samples[0] - samples[1])
	var tangent_y : Vector3 = (samples[2] - samples[0]) if clamped_ty < 0.5 else (samples[0] - samples[2])
...
func _draw_curve_preview(entry_index : int) -> void:
	...
	for i in PREVIEW_STEPS:
		...
		var p0 : Vector3 = _surface_point(tx, t0) + _surface_normal(tx, t0) * curve.sample(t0) * VALUE_VISUAL_SCALE
		var p1 : Vector3 = _surface_point(tx, t1) + _surface_normal(tx, t1) * curve.sample(t1) * VALUE_VISUAL_SCALE
...
func _write_dragged_handle(handle_id : int, world_pos : Vector3) -> void:
	...
	var tx := _entry_lane_tx(entry_index)
	var ty := _clamped_curve_offset(curve, point_index, _closest_ty(world_pos, tx))
	var base := _surface_point(tx, ty)
	var normal := _surface_normal(tx, ty)
	var value := (world_pos - base).dot(normal) / VALUE_VISUAL_SCALE
...
func _screen_distance_to_curve(entry_index : int, cam : Camera3D, mouse_pos : Vector2) -> Dictionary:
	...
	for i in PREVIEW_STEPS + 1:
		...
		var world : Vector3 = _surface_point(tx, offset) + _surface_normal(tx, offset) * curve.sample(offset) * VALUE_VISUAL_SCALE
