...
func append_message(sender_id: int, text: String) -> void:
	var name: String = game_manager._player_display_name(sender_id)
...
func update_race_overlay() -> void:
	...
	player_names[local_id] = game_manager._player_display_name(local_id)
	...
	for peer_data in remote_peers:
		...
		if peer_id >= 0:
			player_names[peer_id] = game_manager._player_display_name(peer_id)
