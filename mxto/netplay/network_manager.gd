...
func collect_inputs() -> Array:
	if is_server:
		...
		sent_input_times[local_tick] = 0.001 * Time.get_ticks_msec()
	...
	if not is_server:
		...
		if acks.has(multiplayer.get_unique_id()):
			...
			if sent_input_times.has(ack_tick):
				var sample := 0.001 * Time.get_ticks_msec() - int(sent_input_times[ack_tick])
				if rtt_ms == 0.0:
					rtt_ms = sample
				else:
...
func _update_desired_ahead() -> void:
	desired_ahead_ticks = ((rtt_ms * 0.5) + JITTER_BUFFER) / base_wait_time

func _adjust_time_scale() -> void:
	if is_server:
		return
	var current_ahead := float(local_tick - server_tick) * base_wait_time
	var desired_ahead := (rtt_ms * 0.5) + JITTER_BUFFER
