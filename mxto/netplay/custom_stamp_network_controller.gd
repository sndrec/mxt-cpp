...
func _receive_custom_stamp_manifest(player_id: int, manifest: Array) -> void:
	if network_manager.race_active:
		return
	var sender_id := multiplayer.get_remote_sender_id()
	if network_manager.is_server and sender_id != 0:
		return
...
func send_manifests_to_peer(peer_id: int) -> void:
	for player_id in custom_stamp_manifests.keys():
		_receive_custom_stamp_manifest.rpc_id(peer_id, int(player_id), custom_stamp_manifests[player_id])
