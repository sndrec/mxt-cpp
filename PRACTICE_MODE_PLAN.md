...
- 2026-08-20 — Phase F: added Off/Compact/Expanded Practice telemetry, a 10 Hz visible-only native sampling path, compact and expanded readouts over the fixed native telemetry sample, and diagnostic counters proving that the Off path does not sample or format telemetry. The Practice HUD was resized for the expanded presentation. `scons target=template_release -j4` passed, and an isolated graphical launch remained alive through the smoke window without script, parser, runtime-call, or import-file errors.
- 2026-08-20 — Phase G: added replay-resume eligibility with explicit disabled reasons, a confirmation dialog with default-enabled `Keep Original as Ghost`, decoded-frame caching, exact full-state capture, canonical-prefix seeding, and an exact recorded-roster race startup path. Resume transfers the focused racer to local control, converts every other racer to native CPU control without changing order or machine state, restores race-result companion state, and enters Practice at 0.00x. The optional Original ghost runs the complete recorded roster and inputs in an isolated `GameSim`, fast-forwards to the branch cursor, fades at source completion, and participates in Practice rewind/savestate restoration. Practice exports now identify their authoritative multi-racer source directly. `scons target=template_release -j4` passed, and an isolated graphical launch remained alive through the smoke window with no script, parser, runtime-call, or import-file errors. Focused replay-resume and branch-reproduction checks remain in Phase H.
