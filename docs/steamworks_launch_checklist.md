...
On the current Windows deployment path, double-click `manage_steam_backend.bat`
and use **DEPLOY VERIFIER** after setting the associated game build live. The
manager builds the release-exported runtime, assembles and atomically swaps the
pinned verifier bundle, restarts its Scheduled Tasks, and checks the loopback
service plus public Cloudflare route. Use **SYNC BOARDS TO STEAM + DEPLOY** only
when the checked-in leaderboard manifest changes. The manual building blocks
remain `services/leaderboard_verifier/build_windows_bundle.ps1`, the bundle's
`start_windows_bundle.ps1`, and `cloudflared.example.yml`. Store the tunnel
credential, publisher key, leaderboard-ID map, and curated-package map outside
both the repository and bundle.
