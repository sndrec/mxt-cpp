...
public:
	...
	if (segment.checkpoint_count == 0)
	continue;
	if (!segment.bounds.has_point(in_point))
	continue;
	int start = segment.first_checkpoint;
	int end   = start + segment.checkpoint_count;
	for (int i = start; i < end; ++i)
	{
	const CollisionCheckpoint &cp = checkpoints[i];
	if (!cp.start_plane.is_point_over(in_point))
	continue;
	if (cp.end_plane.is_point_over(in_point))
	continue;
	godot::Vector3 avg_pos = (cp.position_start + cp.position_end) * 0.5f;
	float max_x_radius = fmaxf(cp.x_radius_start, cp.x_radius_end);
	float max_y_radius = fmaxf(cp.y_radius_start, cp.y_radius_end);
	float max_total_radius = fmaxf(max_x_radius, max_y_radius);
	float radius = (cp.position_end - cp.position_start).length_squared() + max_total_radius * max_total_radius;
