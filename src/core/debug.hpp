...
namespace DEBUG
{
	...
	extern int YAW_TRACE_CAR_INDEX;
	extern int YAW_TRACE_TICK_START;
	extern int YAW_TRACE_TICK_END;
	...
	inline bool yaw_trace_filter_matches(int car_index, uint32_t tick)
	{
		if (YAW_TRACE_CAR_INDEX >= 0 && YAW_TRACE_CAR_INDEX != car_index) {
			return false;
		}
		if (YAW_TRACE_TICK_START >= 0 && tick < static_cast<uint32_t>(YAW_TRACE_TICK_START)) {
			return false;
		}
		if (YAW_TRACE_TICK_END >= 0 && tick > static_cast<uint32_t>(YAW_TRACE_TICK_END)) {
			return false;
		}
		return true;
	}
