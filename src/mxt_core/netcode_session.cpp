...
#include "mxt_core/auth_input_delta_low_entropy_s12_zstd_dictionary.h"
#include "mxt_core/auth_input_delta_low_entropy_s15_zstd_dictionary.h"
#include "mxt_core/auth_input_delta_low_entropy_surface_alt_zstd_dictionary.h"
#include "mxt_core/auth_input_delta_low_entropy_surface_s11_zstd_dictionary.h"
...
#include "mxt_core/auth_input_delta_low_entropy_surface_fallback_s3_zstd_dictionary.h"
...
godot::PackedByteArray NetcodeSession::build_authoritative_input_packet(int last_tick, int max_frame_count, int race_phase) const
{
	...
	for (uint8_t sublayout = MXT_AUTH_INPUT_LOW_ENTROPY_CURRENT; sublayout <= MXT_AUTH_INPUT_LOW_ENTROPY_MAX_SUBLAYOUT; ++sublayout) {
		...
		if (low_entropy_candidate_raw.size() > 0) {
			...
			candidate = compress_auth_input_with_dict(low_entropy_candidate_raw, false, false, false, false, !default_alt && !default_s1 && !default_s11 && !default_s12 && !default_s15, false, false, false, false, default_alt && !default_s12 && !default_s15, false, default_s1, default_s11, default_s12, default_s15);
			if (candidate.size() > 0 && (compressed.size() <= 0 || candidate.size() < compressed.size())) {
				compressed = candidate;
			...
			candidate = compress_auth_input_with_dict(low_entropy_candidate_raw, false, false, false, false, false, false, !surface_alt && !surface_s11, false, false, false, surface_alt && !surface_s11, false, false, false, false, surface_s11);
			if (candidate.size() > 0 && (compressed.size() <= 0 || candidate.size() < compressed.size())) {
				compressed = candidate;
			...
			candidate = compress_auth_input_with_dict(low_entropy_candidate_raw, false, false, false, false, false, false, false, !fallback_alt && !fallback_s3 && !fallback_s6, fallback_alt && !fallback_s3 && !fallback_s6, false, false, false, false, false, false, false, fallback_s3, fallback_s6);
			if (candidate.size() > 0 && (compressed.size() <= 0 || candidate.size() < compressed.size())) {
				compressed = candidate;
	...
	for (int l = 0; l < 8; ++l) {
		...
		if (layout == AUTH_INPUT_LAYOUT_BITPACKED_BUTTONS_ZERO_BITMAP) {
			...
			for (uint8_t sublayout = MXT_AUTH_INPUT_LOW_ENTROPY_CURRENT; sublayout <= MXT_AUTH_INPUT_LOW_ENTROPY_MAX_SUBLAYOUT; ++sublayout) {
				...
				const bool fallback_s3 = auth_input_low_entropy_fallback_s3_sublayout(sublayout);
				const bool fallback_s6 = auth_input_low_entropy_fallback_s6_sublayout(sublayout);
				const PackedByteArray compressed_surface_fallback = compress_auth_input_with_dict(candidate_raw, false, false, false, false, false, false, false, !fallback_alt && !fallback_s3 && !fallback_s6, fallback_alt && !fallback_s3 && !fallback_s6, false, false, false, false, false, false, false, fallback_s3, fallback_s6);
