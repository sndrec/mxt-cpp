...
constexpr int MXT_NET_AUTHORITATIVE_INPUT_PACKET_HEADER_BYTES = 5;
...
constexpr uint8_t MXT_NET_AUTH_MODE_MASK = 0x7f;
...
{
	return (mode & MXT_NET_AUTH_MODE_MASK) <= MXT_NET_AUTH_MODE_BITPACKED_DICT_ZSTD;
}
...
PackedByteArray compress_auth_input_with_dict(const PackedByteArray& raw)
{
	...
	ZSTD_CDict* cdict = auth_input_zstd_cdict();
	if (!cctx || !cdict) {
		return PackedByteArray();
	}
...
PackedByteArray decompress_auth_input_with_dict(const PackedByteArray& compressed, int raw_size)
{
	if (raw_size <= 0 || compressed.size() <= 0) {
		return PackedByteArray();
	}
	ZSTD_DCtx* dctx = auth_input_zstd_dctx();
	ZSTD_DDict* ddict = auth_input_zstd_ddict();
	if (!dctx || !ddict) {
		return PackedByteArray();
	}
	PackedByteArray out;
...
godot::PackedByteArray NetcodeSession::build_authoritative_input_packet(int last_tick, int max_frame_count, int race_phase) const
{
	...
	if (max_frame_count <= 0 || last_tick < 0) {
		...
		writer.write_u8(0);
		writer.write_u8(static_cast<uint8_t>(MXT_NET_AUTH_MODE_PACKED_PLAIN_ZSTD | ((race_phase & 1) ? MXT_NET_AUTH_PHASE_BIT : 0)));
		return writer.to_pba();
	}
	...
	int compression_mode_pos = writer.pos;
	if (!writer.write_u8(MXT_NET_AUTH_MODE_PACKED_PLAIN_ZSTD)) {
		return PackedByteArray();
	}
	...
	writer.data[compression_mode_pos] = static_cast<uint8_t>(compression_mode | ((race_phase & 1) ? MXT_NET_AUTH_PHASE_BIT : 0));
...
godot::Dictionary NetcodeSession::store_authoritative_input_packet(godot::PackedByteArray packet, int expected_race_phase)
{
	...
	PacketReader reader(packet);
	uint8_t count = 0;
	uint8_t mode_phase = MXT_NET_AUTH_MODE_PACKED_PLAIN_ZSTD;
	uint32_t packed_first_tick = 0;
	if (!reader.read_u24(packed_first_tick) || !reader.read_u8(count) || !reader.read_u8(mode_phase)) {
		return stats;
	}
	if (((mode_phase & MXT_NET_AUTH_PHASE_BIT) ? 1 : 0) != (expected_race_phase & 1)) {
		stats["stale"] = true;
		stats["valid"] = true;
		return stats;
	}
	const uint8_t compression_mode = mode_phase & MXT_NET_AUTH_MODE_MASK;
	...
	if (auth_input_mode_uses_dict(compression_mode)) {
		raw = decompress_auth_input_with_dict(compressed, raw_size);
	} else {
...
godot::Dictionary NetcodeSession::debug_compare_authoritative_input_packet_sizes(int last_tick, int max_frame_count, int race_phase) const
{
	...
	const AuthInputLayout layouts[4] = {
